...
from datasets.augmentations import Resize, Compose, ColorJitter, RandomHorizontalFlip, RandomCrop, RandomScale, \
    RandomRotation
...
class IRSeg(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # combined_image = Image.new('RGB', (1600, 1200))
        # combined_image.paste(transforms.ToPILImage()(sample['image']), (0, 0))  # 将 image1 粘贴到左上角
        # combined_image.paste(sample['depth'], (480, 0))  # 将 image2 粘贴到右上角
        # combined_image.paste(sample['label'], (960, 0))  # 将 image3 粘贴到右上角
        # combined_image.paste(sample['bound'], (0, 600))  # 将 image4 粘贴到左下角
        # combined_image.paste(sample['edge'], (480, 600))  # 将 image5 粘贴到右下角
        # combined_image.paste(sample['binary_label'], (960, 600))  # 将 image6 粘贴到右下角
        # combined_image.show()
        return sample
